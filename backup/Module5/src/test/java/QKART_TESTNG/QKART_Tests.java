...
public class QKART_Tests {
    ...
    public void TestCase02(String userName,String password) throws InterruptedException {
        ...
        assertFalse(status,"Failed to show the error message of re-registration");
    ...
    public void TestCase03(String yonex,String gesundheit) throws InterruptedException {
        boolean status;
         logStatus("Test Case 3", "Start TestCase: Verify functionality of search box", "DONE");
         //takeScreenshot(driver, "StartTestCase", "TestCase1");
        ...
        //    return false;
        //}
        assertNotNull(searchResults,"There were no results for the given search string");
        for (WebElement webElement : searchResults) {
            ...
            //            "FAIL");
               // return false;
            //}
            assertTrue(elementText.toUpperCase().contains("YONEX"),"Test Results contains un-expected values");
        ...
        //    return false;
        //}
        sa.assertTrue(status,"Invalid keyword returned results");
        ...
        assertTrue(homePage.isNoResultFound(),"Test case fail. Expected: no results");
        logStatus("TestCase 3", "Test Case PASS. Verified that no search results were found for the given text","PASS");
    }

    @Test(description="Verify the existence of size chart for certain items and validate cntents of size chart", priority=4, groups = {"Regression_test"})
    @Parameters({"shoes"})
     ...
     * working correctly
     */
    @Test(description="Verify that a new user can add multiple products to the cart and checkout", priority=5, groups = {"Sanity_test"})
     ...
     */
    @Test(description="Verify that contents of the cart can be edited", priority=6, groups = {"Regression_test"})
    ...
    @Test(description="Verify that the contents added to cart are saved against the user's login details", priority=7, groups = {"Regression_test"})
    ...
    public void TestCase08 (String userName,String password,String stylecon,String address) throws InterruptedException {
        ...
        assertTrue(status,"having issue in getting message");
        logStatus("End TestCase",
                "Test Case 8: Verify that insufficient balance error is thrown when the wallet balance is not enough: ",
                status ? "PASS" : "FAIL");
    ...
    @Test(dependsOnMethods={"TestCase10"},description="Verify that product added to cart is available when a new tab is opened", priority=10, groups = {"Regression_test"})
    ...
    @Test(description="Verify that the Privacy Policy, About Us are displayed correctly ", priority=9, groups = {"Regression_test"})
    @Parameters({"userName","password"})
